...
---
## 2. JSON schema for carousel manifest (example)
Save as `schemas/carousel_manifest.schema.json`

```json
{
  "$schema": "http://json-schema.org/draft-07/schema#",
  "title": "Carousel Manifest",
  "type": "object",
  "required": ["id","title","slides","created_at"],
  "properties": {
    "id": {"type": "string"},
    "title": {"type": "string"},
    "niche": {"type": "string"},
    ...
    "slides": {
      ...
      "items": {
        "type":"object",
        "required":["index","text","image_candidates"],
        "properties": {
          "index":{"type":"integer"},
          "text":{"type":"string"},
          "image_candidates": {
...
---
## 3. Example manifest (workflows/sample_workflow.json)
```json
{
  "id": "car-20251110-001",
  "title": "5 Facts About AI",
  "niche": "technology",
  "author": "selto",
  "created_at": "2025-11-10T10:00:00Z",
  "slides": [
    {"index":1, "text":"Fact 1 text", "image_candidates":["pexels_123.jpg","pexels_456.jpg"], "layout":"img-full"},
    {"index":2, "text":"Fact 2 text", "image_candidates":["pexels_789.jpg","pexels_101.jpg"], "layout":"img-left"},
    {"index":3, "text":"Fact 3 text", "image_candidates":["pexels_102.jpg","pexels_103.jpg"], "layout":"img-right"},
    {"index":4, "text":"Fact 4 text", "image_candidates":["pexels_104.jpg","pexels_105.jpg"], "layout":"img-full"},
    {"index":5, "text":"Fact 5 text", "image_candidates":["pexels_106.jpg","pexels_107.jpg"], "layout":"img-full"}
  ],
  "metadata": {"platforms":["instagram","x"]}
}
```

---
## 4. Test script (tests/test_render.py)
Python example that validates manifest against schema and runs a mock render function.

```python
#!/usr/bin/env python3
import json, jsonschema, sys, pathlib

SCHEMA_PATH = pathlib.Path(__file__).resolve().parent.parent / "schemas" / "carousel_manifest.schema.json"
MANIFEST_PATH = pathlib.Path(__file__).resolve().parent.parent / "workflows" / "sample_workflow.json"
...
def mock_render(manifest):
    # simple smoke test: ensure 5 slides present and image candidates exist
    assert len(manifest['slides']) == 5, "manifest must contain 5 slides"
    for s in manifest['slides']:
        assert s.get('image_candidates'), f"slide {s['index']} missing image candidates"
    return True
...
def main():
    ...
    try:
        ...
        ok = mock_render(manifest)
        print('OK') if ok else sys.exit(2)
...
---
## 5. GitHub Actions CI example (.github/workflows/ci.yml)
This workflow runs schema validation and tests on push to main or PRs.
